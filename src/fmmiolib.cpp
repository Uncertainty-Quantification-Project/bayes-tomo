...
void Data::load(const char *filename,const int& n1, const int& n2, const int&n3, const double& s) 
{
  FILE * file;
  long lsize;
  int Ntot;
  ...
  if ( lsize/sizeof(float) == Ntot ){
    for (k=0; k<Ntot; k++){
      fread(&tmp, sizeof(float), 1, file);
      dat.push_back((double)tmp);
    }
    fclose(file);
  }
  else{
    std::cout << "File error: wrong number of entries\n";
    std::cout << "File size after header is " << lsize/sizeof(float) << "\n";
    std::cout << "It should be " << Ntot << "\n";
...
void Data::save(const char *filename,const int& n1, const int& n2, const int&n3, const double& s) 
{
  FILE * file;
  int k;
  float tmp;
...
/// member function that uses the data stored in dat, based on a given (coarse=bruto) grid specified by Nx, Ny, Nz and h, to generate a new Data object containing interpolated values of dat on a refined grid.
...
void locations::Location::findind(Grid& grd)
{
  ...
  ind = i + j*grd.Nx + k*grd.Nx*grd.Ny;*/
